...
const CATALOG: ModelEntry[] = [
  {
    ...
    provider: "gemini",
    model: GEMINI_MODEL,
    apiKeyEnv: process.env.GEMINI_API_KEY ? "GEMINI_API_KEY" : "GEMINI_API_KEY_1",
    hasVision: true,
    limitations: {
...
const hasKey = (e: ModelEntry) => (process.env[e.apiKeyEnv] || "").trim().length > 0;

export function buildChain(): ModelEntry[] {
  const available = CATALOG.filter(hasKey);
  const order = (process.env.MODEL_CHAIN || "").split(",").map((s) => s.trim()).filter(Boolean);

  if (order.length) {
    const chain = order.map((id) => available.find((e) => e.id === id)).filter((e): e is ModelEntry => Boolean(e));
  ...
  return [...available.filter((e) => e.provider === pref), ...available.filter((e) => e.provider !== pref)];
}
